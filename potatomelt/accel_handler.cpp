// his module interfaces with the accelerometer to provide current G-force level

// wo breakout boards using the H3LIS331DL accelerometer have been tested / verified to work

// parkfun's H3LIS331DL breakout: https:// ww.sparkfun.com/products/14480 (3v part)
// equires using  to interface with 5v arduino

// dafruit's HSLIS311 breakout includes a level converter - is a single component solution:
// ttps:// ww.adafruit.com/product/4627

// pdate ACCEL_I2C_ADDRESS in accel_handler.h with I2c address (Adafruit and Sparkfun boards are different!)

// parkfun's LIS311 library is used (also works for Adafruit part)
...
void init_accel() { 
  ...
  Wire.setClock(400000);  // ncrease I2C speed to reduce read times a bit
                          // alue of 400000 allows accel read in ~1ms and
                          // s verfied to work with Sparkfun level converter
                          // some level converters have issues at higher speeds)
  
  xl.setI2CAddr(ACCEL_I2C_ADDRESS);
  xl.begin(LIS331::USE_I2C);

  // ets accelerometer to specified scale (100, 200, 400g)
  xl.setFullScale(ACCEL_RANGE);
}

// eads accel and converts to G's
// CCEL_MAX_SCALE needs to match ACCEL_RANGE value
...
float get_uncorrected_rpm() {
  float rpm;
  // se of absolute makes it so we don't need to worry about accel orientation
  // alculate RPM from g's - derived from "G = 0.00001118 * r * RPM^2"
